...
public class IntermediateConditionalEventBehavior extends IntermediateCatchEventActivityBehavior implements ConditionalEventBehavioral {
  ...
  public void execute(final ActivityExecution execution) throws Exception {
    ...
    if (conditionalEvent.evaluate(execution, execution)) {
      ...
      collectExecutions((ExecutionEntity) execution, executions);

      for (ExecutionEntity executionEntity : executions) {
        EventSubscriptionEntity.createAndInsert(executionEntity, EventType.CONDITONAL, conditionalActivity);
      }
    }
  }

  protected void collectExecutions(ExecutionEntity execution, List<ExecutionEntity> executions) {
    if (execution.getParent() != null) {
      collectExecutions(execution.getParent(), executions);
    }
